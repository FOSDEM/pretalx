...
class Submission(GenerateCode, PretalxModel):
    ...
    def _set_state(self, new_state, force=False, person=None):
        ...
        if force or new_state in valid_next_states:
            ...
            # note that stands and devrooms have no tracksetting, nor track
            # so we skip the test in that case
            if self.track and hasattr(self.track, 'tracksettings') :
                ...
                if person not in self.track.tracksettings.manager_team.members.all() and new_state==SubmissionStates.ACCEPTED and not person.is_administrator:
                    raise SubmissionError(
                            "Talks can only be accepted by track/devroom manager or admin.")
